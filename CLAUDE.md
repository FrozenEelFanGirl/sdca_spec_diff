# SDCA SpecDiff

Compare two versions of the MIPI SDCA specification and produce annotated interleaved diffs showing exactly what changed between revisions.

## Setup

```bash
python -m venv .venv
.venv\Scripts\pip install -r requirements.txt    # Windows
# .venv/bin/pip install -r requirements.txt      # macOS/Linux
```

All subsequent commands use `.venv/Scripts/python` (Windows) or `.venv/bin/python` (macOS/Linux).

## Quick Start

```bash
# Stage 1 — one-time setup
python -X utf8 scripts/init_config.py --clear
python -X utf8 scripts/extract_all.py

# Stage 2 — per-topic comparison
python -X utf8 scripts/search_sections.py "<keyword>"
# edit report: remove ✓ from unwanted sections
python -X utf8 scripts/compare_sections.py doc/comparison_<base>_<comp>/index/<keyword>.md
# manually annotate changes in the generated comparison files
```

## Configuration

`scripts/config.py` — shared module, `load_config()` returns typed `Config` with all paths.

`scripts/config.json` — generated by `init_config.py`, consumed by all other scripts.

```python
from config import load_config
cfg = load_config()
# cfg.base.docx, cfg.base.sections_dir, cfg.base.index_file, ...
# cfg.comparison.docx, cfg.comparison.sections_dir, ...
# cfg.comparison_dir, cfg.mapping_file
```

## Stage 1: Preparation

### init_config.py

Scans `doc/sources/` for `.docx` files, reads cover page + core.xml metadata, assigns the newer version as base and older as comparison. Generates `scripts/config.json` and creates output directory structure.

```
python -X utf8 scripts/init_config.py                  # auto-detect
python -X utf8 scripts/init_config.py --clear           # also clear old output
python -X utf8 scripts/init_config.py --base <p> --comp <p2>  # explicit versions
```

Version is parsed from the cover page ("Version X.Y" + optional "Revision Z" → `vXpYrZ`). Warns if core.xml metadata contradicts the cover page.

### extract_all.py

Orchestrates the full extraction pipeline:
1. `extract_index` for base and comparison versions
2. Full section extraction (sections, tables, images) for both versions
3. `map_sections --mode heading` — full section mapping (`full_mapping.json`)
4. `map_sections --mode content` — content fingerprints for all matched pairs (`content_fingerprints.json`)

```
python -X utf8 scripts/extract_all.py
```

### extract_index.py

Builds a master index (`index.json`) mapping every section, figure, and table number to its heading/caption.

```
python -X utf8 scripts/extract_index.py --from-config [base|comp]
python -X utf8 scripts/extract_index.py <docx_path> --output <path>
```

### extract_section.py

Extracts a single section, including its tables and figures.

```
python -X utf8 scripts/extract_section.py --from-config [base|comp] <section_id>
python -X utf8 scripts/extract_section.py <docx_path> <section_id> --output-dir <dir>
```

`<section_id>` can be a section number (`"5.1.2"`) or heading keyword (`"Smart Amp"`).

### map_sections.py

Two modes for mapping sections between base and comparison versions:

**Heading mode** (`--mode heading`): compares index headings, outputs mapping table with similarity scores:
```
python -X utf8 scripts/map_sections.py --mode heading --filter UAJ
python -X utf8 scripts/map_sections.py --mode heading --output mapping.json
```

**Content mode** (`--mode content`): compares extracted section body text, outputs structural fingerprint delta:
```
python -X utf8 scripts/map_sections.py --mode content --id 5.11.3
python -X utf8 scripts/map_sections.py --mode content --id 5.11.3 --comp-id 5.5.1
```

Mapping verdicts: `exact_match` (≥0.95), `fuzzy_match` (≥0.75), `new_section` (<0.75), `deleted_in_base`.

JSON field names use `base_`/`comp_` prefixes: `base_num`, `comp_num`, `base_heading`, etc.

## Stage 2: Comparison

### search_sections.py

Full-text search in the base docx body text. Word-level matching with prefix support (sequence ↔ sequences). AND semantics: all query words must match.

```
python -X utf8 scripts/search_sections.py "<keyword>"
```

Output: Markdown table with `✓` checkboxes, saved to `<comparison_dir>/index/<keyword>.md`. Columns: `Compare?`, `Section`, `Heading`, `Matches`.

Remove `✓` from unwanted rows to exclude sections from comparison.

### compare_sections.py

Reads a search report and generates interleaved comparison files.

```
python -X utf8 scripts/compare_sections.py <report.md>
```

If `<report.md>` is omitted, compares ALL sections from the mapping.

For each checked section:
- Copies base content to `comparison/sections/<num>_<heading>_comparison.md`
- Pre-fills the comparison counterpart in blockquotes
- Annotation header shows mapping info (`Mapped from comparison §X.Y`, `New in this version`, etc.)

Generated file format (interleaved — comparison blocks follow their base counterparts):
```
> **Mapped from comparison §X.Y**
>

(Base paragraph)

> **vX.Y:**
> (Comparison paragraph in blockquote — aligned by content similarity)

(Base paragraph)

> **vX.Y:**
> (Comparison paragraph)
```

Each comparison block is prefixed with `> **vX.Y:**` showing the comparison version
(with `.` replacing the internal `p` separator, e.g. `v1p0` → `v1.0`).
Blocks that are identical between versions are annotated `> *Unchanged.*`.

**Blank-line-before-list rule**: When a comparison blockquote contains a list
(numbered or bullet), a blank `>` line MUST precede the first list item
so that markdown renderers recognize the list as a new block:

```
> **v1.0:**
>
> 1. First list item
> 2. Second list item
```

Without the blank `>` line, some renderers flatten nested list numbering.

**Editing comparison files** (refine the generated interleaved format):

- **Move annotation**: `> **Moved from comparison §X.Y, Updated**`
- **Paragraphs**: comparison text immediately follows each paragraph
- **Lists**: comparison blockquote follows the entire list (never insert between items)
- **Unchanged steps**: `> N. SAME AS N`
- **Sub-items in blockquotes**: 5 spaces after `>` (`>     1. Sub-step`)
- **Sub-items in base**: 4 spaces per indent level
- Changed text in **bold**, removed text in ~~strikethrough~~

## Other Scripts

### extract_figure.py / extract_table.py

Extract a single figure or table by number.

```
python -X utf8 scripts/extract_figure.py <docx_path> <num> [--output-dir DIR]
python -X utf8 scripts/extract_table.py <docx_path> <num> [--output-dir DIR]
```

### diff_images.py

Pixel-diff two figures, output ratio and verdict.

```
python -X utf8 scripts/diff_images.py <image1> <image2> [--diff-output DIFF.png]
```

## Directory Layout

```
doc/
  sources/                              # Original .docx files
  output_<base>/                        # Extractions from base (newer) version
    sections/                           # <num>_<heading>.md
    tables/                             # Table<num>.md
    images/                             # Figure<num>.png
    index/                              # index.json
  output_<comp>/                        # Extractions from comparison (older) version
    (same structure)
  comparison_<base>_<comp>/             # Interleaved comparison files
    sections/                           # <num>_<heading>_comparison.md
    tables/                             # (reserved for table diffs)
    images/                             # (reserved for image diffs)
    index/                              # mapping + search reports + fingerprints
scripts/
  config.py                             # Shared config loader
  config.json                           # Generated by init_config.py
  init_config.py                        # Stage 1.1: discover versions, generate config
  extract_all.py                        # Stage 1.2+1.3: full extraction pipeline
  extract_index.py                      # Build master index
  extract_section.py                    # Extract single section
  extract_figure.py                     # Extract single figure
  extract_table.py                      # Extract single table
  map_sections.py                       # Map + fingerprint sections
  diff_images.py                        # Pixel-diff two figures
  search_sections.py                    # Stage 2.1: full-text keyword search
  compare_sections.py                   # Stage 2.2: generate comparison files
```

## Dependencies

- Python 3.14 with `lxml` (`pip install python-docx`)
- PIL/Pillow for EMF to PNG conversion
- No python-docx API used directly — raw lxml for OOXML access
- The `-X utf8` flag is required on Windows (spec contains non-ASCII characters)
